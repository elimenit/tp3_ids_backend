...
from database.public.reviews import (
    db_get_reviews, db_get_review, db_create_review, db_update_review, db_delete_review
)
from services.public.reviews import validation_create_review, validation_update_review
...
def create_review():
    ...
    if not validation_create_review(user_id, reservation_id, description, stars):
        return error_response("Campos inválidos", "Revisá que todos los campos sean correctos y que las estrellas esten entre 1 y 5", 400)

    try:
        new_id = db_create_review(user_id, reservation_id, description, stars)
    except Exception as e:
...
def update_review(review_id: int):
    ...
    if user_id is None:
        return error_response("Campo inválido", "Se requiere user_id para verificar que la reseña te pertenece", 400)
    ...
    try:
        updated = db_update_review(review_id, user_id, description, stars)
    except Exception as e:
...
def delete_review(review_id: int):
    ...
    try:
        deleted = db_delete_review(review_id, user_id)
    except Exception as e:
